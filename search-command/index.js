                    ...
                    if (err){
                        ...
                        job.fetch(function(err){
                            ...
                            splunkjs.Async.whilst(
                                ...
                                function(done) {
                                    job.results({count: myCount,offset:myOffset}, function(err, resultFromSplunk) {
...
search -u admin -p P@ssw0rd -h localhost --port 8089 --query "ssh* error OR *fail* OR severe |rex field=_raw "(?<val_ignore_1>.*)\ for (?<raw_user>.*)\ from (?<IP>.*)\ port (?<val_ignore_2>.*)" | eval User=replace(raw_user, "invalid user ", "") | table User IP" -d
